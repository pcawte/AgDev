...
linker_script: $(STATIC_FILES) $(LINKED_FILES) $(SHARED_FILES)
	...
	$(Q)$(call APPEND)
	$(Q)$(call APPEND,order .header$(comma) .icon$(comma) .init.libs$(comma) .init.args$(comma) .init.bss$(comma) .libs$(comma) .init.clock$(comma) .init.reruncheck$(comma) .init$(comma) .fini$(comma) .text$(comma) .data$(comma) .rodata)
